%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% An introductory course for machine learning, EEIA Godomey 2024 version
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Find $x$, the weights and biases of the Neural Network (NN) so that the function $y(\cdot \mid x)$ approaches the observed data
\vskip\baselineskip
...
\\
{\small
where $y(e \mid x) = X^L \Phi^L\left(\ldots \Phi^2(X^2 \Phi^1(X^1 e)) \ldots\right)$, \\
$X^1,\ldots,X^L$ the weights/biases arranged as matrices for each of the $L$ layers, 
$\Phi^i$ is a vector of activation functions (reLU, linear, sigmoid, leaky reLU) for the $i$-th layer.
} % end small
\end{frame}
...
%\begin{center}
%\includegraphics[width=0.5\textwidth]{neuralnet3-4-1-classif-crop.pdf}\\
%\end{center}
\begin{itemize}
\item $y(e;x)$: output of the NN $\in \mathbb R ~\ne~ \{0,1\} $: the space of the function to learn is discrete, that of the NN is continuous \Sadey[1.5][red!60!white]
\item Trick: predict the probability that $t(e)=1$. This probability is in $[0,1]$ which is continuous ... yet bounded \Sey[1.5][red!30!white]
...
to get $y(\cdot ; x) \in [0,1]$\Tongey[1.5][green!60!white] 
...
% I don't keep this version although it is proved in the EMSE course: 
% because the match between the left curve and the formula is not direct.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{frame}%[allowframebreaks]
%\frametitle{Gradient convergence : summary}
%$f(x) = \frac{1}{2} x^\top H x$ in $n=10$ dimensions, $H>0$, not aligned with the axes, condition number = 10.
%\begin{center}
%\mbox{
%\begin{minipage}[c]{0.37\textwidth}
...
%\hspace{0.5cm}
%\begin{minipage}[c]{0.57\textwidth}
%\cite{vanderbergheOptimCourse,ravikumar17,mu_li_video19}: on convex and L-strongly smooth functions,
%gradient search with constant and sufficiently small scale factor achieves after $T$ iterations
%\begin{equation}
%f(x^T) - f(x^\star) \le \frac{\lVert x^0-x^\star \rVert}{2 \bar{\alpha} T}
%\label{eq:grad_fconv}
%\end{equation}
%i.e., $\mathcal O(1/T)$ convergence.
%To achieve $f(x^T)-f(x^\star) < \varepsilon$, $T>\mathcal O(1/\varepsilon)$
%\vskip\baselineskip
%\end{minipage}
%} % end mbox
%\end{center}
